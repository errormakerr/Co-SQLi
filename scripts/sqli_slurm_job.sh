...
case "${SQLI_MODE:-db-check}" in
    ...
    refactor-smoke)
        ...
        ;;
    rebuild-data)
        "$PYTHON_BIN" "$PROJECT_ROOT/scripts/rebuild_taxonomy_data.py"
        ;;
    full)
        main_args=()
        ...
        ;;
    *)
        echo "Unsupported SQLI_MODE=${SQLI_MODE:-}. Use db-check, generate-smoke, mutation-smoke, refactor-smoke, rebuild-data, or full." >&2
        exit 2
        ;;
esac
