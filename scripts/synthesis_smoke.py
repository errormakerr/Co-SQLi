#!/usr/bin/env python3
"""Validate synthesis boundaries without calling an LLM or training.
...
``src/cosqli/modeling``, and that one database-specific template can be filled and
...
from cosqli.synthesis.template_fillers import SpecificDatabaseTemplateFiller
...
def load_generation_data() -> Tuple[List[Dict[str, Any]], ...]:
    ...
    return (
        read_json_file(str(SOURCE_DATA_DIR / "sql_data_with_injection_point.json")),
        read_json_file(str(SOURCE_DATA_DIR / "payload_template.json")),
        read_json_file(str(SOURCE_DATA_DIR / "schema.json")),
        read_json_file(str(SOURCE_DATA_DIR / "system_table_schema.json")),
        read_json_file(str(SOURCE_DATA_DIR / "system_var.json")),
        read_json_file(str(SOURCE_DATA_DIR / "comment_repository.json")),
    )
...
def validate_layout() -> None:
    ...
    for filename, script_path in expected_modeling.items():
        path = Path(script_path)
        if path.name != filename or path.parent.name != "modeling" or not path.is_file():
            raise RuntimeError(f"Defender modeling path is invalid: {path}")

    retired_paths = (
        PROJECT_ROOT / "src" / "Attacker",
        PROJECT_ROOT / "src" / "Defender",
        PROJECT_ROOT / "src" / "Verifier",
        PROJECT_ROOT / "src" / "modeling",
        PROJECT_ROOT / "src" / "synthesis",
